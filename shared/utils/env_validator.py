...
def validate_environment(env_path: str = None, load_env: bool = True, override: bool = True, env_sample_path: str = None) -> bool:
    """
    Run all environment validation checks.
    
    Args:
        env_path: Path to the .env file to load. If None, looks for:
                  1. .env in current directory
                  2. ../../shared/config/.env (for notebooks)
    ...
    """
    if load_env:
        # Determine env_path if not provided
        if env_path is None:
            from pathlib import Path
            # Try current directory first
            if Path('.env').exists():
                env_path = '.env'
            # Try shared config location (for notebooks)
            elif Path('../../shared/config/.env').exists():
                env_path = '../../shared/config/.env'
            else:
                env_path = '.env'  # fallback
        
        # Determine env_sample_path if not provided
        if env_sample_path is None:
            from pathlib import Path
            # Try current directory first
            if Path('.env.sample').exists():
                env_sample_path = '.env.sample'
            # Try shared config location (for notebooks)
            elif Path('../../shared/config/.env.sample').exists():
                env_sample_path = '../../shared/config/.env.sample'
            else:
                env_sample_path = '.env.sample'  # fallback
